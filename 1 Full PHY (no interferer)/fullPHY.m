...
clear all
tStart = tic;
mcs = [5]; % Vector of MCS to simulate between 0 and 11
...
maxnumberrors = 20*1e3;  % The maximum number of packet errors at an SNR point
maxNumPackets = 20*1e3; % The maximum number of packets at an SNR point
% maxnumberrors = 4*1e2;  % The maximum number of packet errors at an SNR point
% maxNumPackets = 4*1e2; % The maximum number of packets at an SNR point
...
tEnd = toc(tStart);
fname_I = sprintf('sinrPer_Config%d_%s_%s-by-%s_MCS%s_error01_add.mat',cfgHE.AllocationIndex,char(chan),num2str(numTxRx(1)),num2str(numTxRx(2)),num2str(mcs));
